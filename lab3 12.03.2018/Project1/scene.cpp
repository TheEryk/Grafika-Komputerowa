...
{            
	...
	switch(key)
	{
		...
		case 38: {
			rot_x -= 0.3f;
			przesun[0] = glm::vec4(1.0, 0.0, 0.0, -0.5*cos(rot_y));
			przesun[1] = glm::vec4(0.0, 0.0, 0.0, 1.0 - 0.2*sin(rot_x));
			przesun[2] = glm::vec4(0.0, 0.0, 1.0, -0.5*sin(rot_y));
			przesun[3] = glm::vec4(0.0, 0.0, 0.0, 1.0);

			eye = glm::vec3(glm::vec4(eye, 1.0)*przesun);
			break;
		}
		case 39: {
			rot_y += 0.05f; 
...
{	
	...
	obrot[0] = glm::vec4(0.0, 0.0, 0.0, eye.x - cos(rot_y+rot_z));
	obrot[1] = glm::vec4(0.0, 0.0, 0.0, 1.0 - 0.2*sin(rot_x));
	obrot[2] = glm::vec4(0.0, 0.0, 0.0, eye.z - sin(rot_y+rot_z));
	obrot[3] = glm::vec4(0.0, 0.0, 0.0, 1.0);

	center = glm::vec3(glm::vec4(center, 1.0)*obrot);

	glm::mat4 mModelView = glm::lookAt(eye,center,up);
