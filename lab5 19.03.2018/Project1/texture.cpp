...
void glTexture::LoadBMP(char *filename,int &_width, int &_height, char **pixels)
{
	...
	if (fil == NULL) ThrowException((char*)"Brak pliku BMP");
	...
	// read the signature 
	fread(buf,1,2,fil); 	
	if ((buf[0] !=0x42) || (buf[1] != 0x4d)) {fclose(fil); ThrowException((char*)"Invalid signature");}

	// read the filesize
	fread(&file_size,sizeof(file_size),1,fil);
	if (file_size != file_actual_size) {ThrowException((char*)"Invalid file size");}
	...
	// bits per pixel
	fread(buf,1,1,fil);
	if (buf[0] != 24) ThrowException((char*)"Invalid BPP");
	...
	if (bmp_compression != 0) ThrowException((char*)"Invalid Compression");; // invalid compression
