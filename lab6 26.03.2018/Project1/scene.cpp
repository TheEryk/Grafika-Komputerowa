...
void Scene::PreparePrograms()
{
	...
	if (!glIsProgram(program)) {err = 1; ThrowException((char*)"Nie udalo sie utworzyc programu");}
...
{  
	...
	Sphere = new glSphere(2.0, (char*)"textures\\moon.bmp");

	SkyBox = new glSkyBox(25,50, (char*)"textures\\sky.bmp");
	...
	Cube = new glObject();
	Face = new glTexture((char*)"textures\\happy.bmp");
...
{            
	...
	switch(key)
	{
		...
		case 'Z': {Alpha += 0.1f; break; }
		case 'X': {Alpha -= 0.1f; break; }
		
		case 'V': {FogDensity += 0.01f; break; }
		case 'B': {FogDensity -= 0.01f; break; }
		...
		case 65: {Cam_angle -= 0.05f; break;} //A
		case 68: {Cam_angle += 0.05f; break;} //D
	}
...
{	
	...
	glm::mat4 mModelView = glm::lookAt(glm::vec3(Cam_r*sin((Cam_angle)),0.0, Cam_r*cos((Cam_angle))),
										glm::vec3(0.0f),
